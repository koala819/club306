...
export default function record(dataFromLocalStorage, setIsRegistered, nextStep, mailGoogle) {
  ...
  if (mailGoogle) {
    ...
    fetch(`${process.env.CLIENT_URL}/api/auth/recordMemberInfo` || `${process.env.CLIENT_URL2}/api/auth/recordMemberInfo` , options)
      .then(async (response) => {
        if (response.status === 208) {
          console.log('_MembershipContent4_ New member has been created in db supabase with success :)');
          const response = await fetch(`${process.env.CLIENT_URL}/api/sendMail` || `${process.env.CLIENT_URL2}/api/sendMail`, options)
